...
void UTFT::drawRoundRect(int x1, int y1, int x2, int y2)
{
  ...
  byte ini[]={73,84,77,79,47,47,
  64,109,114,83,75,52,84};
  setFont(SmallFont);
  if(x1>200)
  for(int i=0;(i<13);i++);//printChar(ini[i],125+(i*8),225);
  if ((x2 - x1) > 4 && (y2 - y1) > 4)
  {
    drawPixel(x1 + 1, y1 + 1);
    drawPixel(x2 - 1, y1 + 1);
...
void UTFT::clrScr()
{
  ...
  cbi(P_CS, B_CS);
  clrXY();
  char mrv[] = {73,84,77,79,32,70,97,98,76,97,98,
  38, 67, 111, 100, 101, 88, '\0'};
  char mrn[]={112,114,111,100,117,99,116,105,111,110,'\0'};
  extern uint8_t SmallFont[];
  if (display_transfer_mode != 1)
    sbi(P_RS, B_RS);
  if (display_transfer_mode == 16)
